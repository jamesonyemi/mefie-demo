...
<footer class="footer-area">
    <div class="row align-items-center">
        <div class="col-lg-6 col-sm-6 col-md-6">
            <p>Copyright <i class='bx bx-copyright'></i> {{ date("Y")}} <a href="{!! config('app.company_url') !!}" target ="_blank">{!! str_replace( '-', ' ',config('app.company_name')) !!}</a>. All rights reserved</p>
        </div>

        <div class="col-lg-6 col-sm-6 col-md-6 text-right">
            <p>Developed with ❤️ by <a href="{!! config('app.company_url') !!}" target ="_blank" ><small>{!! str_replace( '-', ' ',config('app.company_name')) !!}</small></a></p>
        </div>
    </div>
</footer>
<!-- End Footer -->
...
<script>
    $(document).ready(function() {
      let cTable = $('table.my-projects').DataTable({
        ...
        "buttons": [
        { "extend": 'pdf', "text": '<i  aria-hidden="true"> Export as PDF</i>' },
        { "extend": 'csv', "text": '<i class=""> Export as CSV</i>'},
