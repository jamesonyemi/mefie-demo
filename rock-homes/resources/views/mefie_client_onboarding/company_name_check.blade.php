<script>
 ...
 async function renderUsers() {
    ...
    const url    = `onboarding/sign-up/customers/check-company-name`;
    let company  =  document.querySelector("#company_name");
     ...
     if ( company.value.length <= 0 ) {
        ...
        let htmlSegment = `<div class="my-1 mr-1 row animate__animated animate__headShake">
                                        <span class="badge badge-danger input-group">company name cannot be empty
            ...
            let verify_customer_name_exist = await getUsers(url, company.value);
                
                if ( verify_customer_name_exist.length > 0 ) {
                    
                    verify_customer_name_exist.some(user =>  {
                        if ( (user.company_name === company.value ) ) {
                            
                            document.getElementById("btn-save").setAttribute("disabled", "disabled");

                            let htmlSegment = `<div class="row animate__animated animate__headShake">
                                                <div class="mr-3 email">
                                                    <span class="badge badge-danger">${user.company_name} already taken
                                                        <span class="pt-1 pb-2 mx-1 mb-1 border-danger border-left">
                                                        </span>
                    ...
                    document.getElementById("btn-save").removeAttribute("disabled", "disabled");

                    let htmlSegment = `<div class="row animate__animated animate__shakeY">
                                            <div class="mr-3">
                                                <span class="badge badge-success">${company.value} is available
        ...
        let container = document.querySelector('#check-company-name-exist');
