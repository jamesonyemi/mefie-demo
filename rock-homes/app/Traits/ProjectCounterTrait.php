...
trait ProjectCounterTrait
{
    ...
    public static function projectCounter($view_name = "home")
    {
        ...
        # code...;
        $get_client_project    = DB::table('vw_project_owned_by_clients as a')
                ->where("created_by", Auth::user()->tenant_id);

        $project_count = $get_client_project->select( DB::raw('count(a.pid) as project_counter') );
